/*
The two score loss matrices of the Monte Carlo experiment, built without
fitting anything.

This is applications/abm_system_score_loss.c's procedure with the benchmark
changed. There the score is taken at the US data's estimate; here it is taken
at one simulated replicate's estimate, so the right answer is known - the
configuration that replicate was drawn from. docs/MONTECARLO_VALIDATION.md is
the write-up and says why a copy rather than a shared source: the two pipelines
are independent, and applications/ is not touched by anything here.

For configuration i and replicate n, with theta the benchmark's own cached
estimate,

    q_in = d log L(theta ; y_in) / d theta

evaluated on that replicate's simulated series. Two losses come out of one pass
over the data:

  montecarlo/out/score_loss.csv           q_in' q_in
  montecarlo/out/score_loss_weighted.csv  q_in' [T_sim I_1(theta)]^-1 q_in

The second is Rao's score statistic, which unlike the first does not depend on
how the model is parameterized, and which has a chi-square reference under the
null that the replicate came from the auxiliary model at theta. I_1 is the
observed information at the benchmark's estimate on the benchmark's own series,
so the metric is the curvature of the likelihood at the point the comparison is
centred on. docs/ABM_SYSTEM_SCORE_LOSS.md sets out both in full, including what
the chi-square calibration is and is not worth away from the null.

The benchmark is whichever run montecarlo/benchmark_choice.c picked, read back
through montecarlo/benchmark.h, and its replicate is held out of every
configuration's column for the reason montecarlo/benchmark.h gives.

A replicate whose likelihood is not usable at theta comes back as minus
infinity with a zeroed gradient, which would read as a perfect loss of zero.
That is counted as a missing cell, not as a zero.

Requires out/abm_system_fit_qvarma/ for the benchmark's fit and
dataset/abm_system/ for every series, and rebuilds neither. Nothing printed.
*/
...
#define OUTPUT_PATH_DEFAULT "montecarlo/out/score_loss.csv"
...
/*
The benchmark whose estimate the score is taken at. Unset, it is the US fit and
the real data, which is what the main pipeline runs. Set, it is one simulated
replicate's cached fit and that replicate's own series, which is what
montecarlo/ runs: the same procedure with a known answer substituted for the
real data.

ABM_SYSTEM_BENCHMARK_REPLICATE is then also the replicate index dropped from
every column. Replicate n of every configuration is seed n+1 of the model
(applications/abm_system_simulate.c's "seed = mc + 1", the same seeds for every
configuration), so dropping the row drops every simulation sharing the
benchmark's seed. The benchmark scored against itself would sit at a score of
essentially zero and win by construction.

The information matrix is taken at the benchmark's own estimate on the
benchmark's own series either way, so the weighting is the curvature of the
likelihood at the point the comparison is centred on, whatever that point is.
*/
...
static int trailing_index(const char *name) {
    ...
    assert(underscore && "montecarlo/score_loss: a configuration directory name has no trailing _<N>");
...
static SampleEntry *list_samples(const char *dir, int *count) {
    ...
    assert(handle && "montecarlo/score_loss: cannot open dataset/abm_system/ - "
    ...
    while ((entry = readdir(handle)) != NULL) {
        ...
        if (n == cap) {
            ...
            assert(grown && "montecarlo/score_loss: out of memory listing configurations");
            entries = grown;
        }
        size_t len = strlen(entry->d_name);
        entries[n].name = malloc(len + 1);
        assert(entries[n].name && "montecarlo/score_loss: out of memory copying a configuration name");
...
static int count_replicates(const char *sample) {
    ...
    assert(count > 0 && "montecarlo/score_loss: a configuration directory holds no replicate");
...
static Mat first_replicate(const char *sample) {
    ...
    assert(count > 0 && "montecarlo/score_loss: the first archive of a configuration is empty");
...
static Mat build_information_factor(Vec theta, const QvarmaParams *shape, Mat real_y,
    ...
    assert(status == 0 && "montecarlo/score_loss: the information matrix at theta_US has no "
    ...
    fprintf(manifest, "  observed information at the benchmark estimate, -d2 logL/dtheta dtheta', "
                       "differenced from the analytic gradient over %d periods of its own series\n",
            real_y.c);
    ...
    assert(smallest > floor_value && "montecarlo/score_loss: the information matrix at theta_US is "
    ...
    fprintf(manifest, "  scaled by T_sim / T_benchmark = %d / %d before inversion, so the loss is on the "
                       "chi-square scale\n", sim_periods, real_y.c);
    fprintf(manifest, "  under the null that a replicate came from the auxiliary model at the "
                       "benchmark estimate, "
...
static int fill_column(const char *sample, Vec theta, const QvarmaParams *shape, Mat factor,
    ...
    for (int batch = 0; batch < n_batches; batch++) {
        ...
        for (int b = 0; b < count; b++) {
            /* The benchmark's own seed, left as it was initialised so the drop
               below takes the whole row out of every column. */
            if (replicate[b] < n_replicates && replicate[b] != BENCHMARK_REPLICATE) {
    ...
    int n_missing = 0;
    for (int row = 0; row < n_replicates; row++)
        if (row != BENCHMARK_REPLICATE && MISNAN(AT(plain, row, col + 1))) {
            n_missing++;
            #pragma omp critical
...
int main(void) {
    ...
    assert(n_samples > 0 && "montecarlo/score_loss: no configuration directories under dataset/abm_system/");
    ...
    snprintf(fit_path, sizeof fit_path, "%s/%s/replicate_%03d_%s_fit.json",
             FIT_DIR, BENCHMARK_SAMPLE, BENCHMARK_REPLICATE, SPEC_LABEL);
    int loaded = qvarma_load_params(&real, fit_path);
    assert(loaded && "montecarlo/score_loss: could not load the benchmark fit - run "
                      "us_qvarma_spec_choice.c, or abm_system_fit_qvarma, first");
    ...
    assert(manifest && "montecarlo/score_loss: cannot open the manifest path for writing");
    fprintf(manifest, "%d configurations, %d replicates each, %d periods each, spec %s\n",
            n_samples, n_replicates, sim_periods, SPEC_LABEL);
    fprintf(manifest, "q is the score of the t-QVARMA log-likelihood at the benchmark's estimate, "
    ...
    fprintf(manifest, "benchmark: %s replicate %03d, held out of every column\n",
    ...
    /* The benchmark's own series, which the information matrix is computed on:
       the US block, or the simulated replicate standing in for it. */
    char benchmark_dir[560];
    snprintf(benchmark_dir, sizeof benchmark_dir, "%s/%s", INPUT_DIR, BENCHMARK_SAMPLE);
    Mat real_y = abm_system_read_replicate(benchmark_dir, BENCHMARK_REPLICATE);
    ...
    assert(keep > 0 && "montecarlo/score_loss: every replicate has a missing cell");
    if (keep < n_replicates) {
        int held_out = 1;
        fprintf(manifest, "%d of %d replicates dropped for holding a missing cell\n",
                n_replicates - keep - held_out, n_replicates);
        fprintf(manifest, "replicate %03d held out of every column, the benchmark's own seed\n",
                    BENCHMARK_REPLICATE);
    }
    ...
    assert(plain_frame.r > 0 && "montecarlo/score_loss: the loss table has no rows");
